...
import javaexercise.ownutil.SerializeUtil;
...
public class RedisClient
{
    ...
    private void initialPool()
    {
        ...
        jedisPool = new JedisPool(config, "192.0.6.78", 6379);
    }
    ...
    private void initialShardedPool()
    {
        ...
        shards.add(new JedisShardInfo("192.0.6.78", 6379, "master"));
    ...
    private void operateObject()
    {
        Person person = new Person(100, "rxh");
        jedis.set("person:100".getBytes(), SerializeUtil.serialize(person));

        byte[] rperson = jedis.get("person:100".getBytes());
        Person personValue = (Person) SerializeUtil.unserialize(rperson);
        if (personValue != null)
        {
            System.out.println("获取的对象为 id: " + personValue.getId() + " name: " + personValue.getName());
        } else
        {
